...
namespace :reports do
  ...
  task customer_subscription_events: [:environment] do
    ...
    comments.each do |comment|
      ...
      if !user
        line = "User #{comment.resource_id} not found for ActiveAdminComment #{comment.id}. \n"
      elsif new_plan.hours.to_i > old_plan.hours.to_i and new_plan.amount.to_i != 0 and !user.is_new_in_month?(now)
